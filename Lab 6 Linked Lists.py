...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 12 18:24:08 2024
...
def has_sublist(s, sublist):
    ...
    if s == empty:
        ...
        s_rest = rest(s)
        sub_rest = rest(sublist)
        while sub_rest !=empty:
            if s_rest == empty or first(s_rest) != first(sub_rest):
                return False
            s_rest = rest(s_rest)
            sub_rest = rest(sub_rest)
        return True
...
def count_change(amount, denominations):
    ...
    """
    if amount == 0:
        return 1 #combinations +=1
    if denominations == empty or amount < 0:
        return 0
    largestcoins = count_change(amount - first(denominations), denominations)
    withoutlargestcoins = count_change(amount,rest(denominations))
    return largestcoins + withoutlargestcoins
